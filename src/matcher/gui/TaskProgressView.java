...
public class TaskProgressView extends Control {
    ...
    public TaskProgressView() {
		...
		TaskManager.registerEventListener((task, event) -> Platform.runLater(() -> {
			switch (event) {
			case TASK_STARTED:
				tasks.add(task);
				break;
	...
	private class TaskProgressViewSkin extends SkinBase<TaskProgressView> {
		TaskProgressViewSkin(TaskProgressView progressView) {
			super(progressView);
