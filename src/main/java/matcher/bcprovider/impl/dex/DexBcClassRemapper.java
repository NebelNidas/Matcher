...
public class DexBcClassRemapper {
	public static DexBcClass process(DexBcClass classToRemap, BcClassRemapNameProvider renameProvider) {
		DexBcClassRemapper remapper = new DexBcClassRemapper(classToRemap, renameProvider);
		return remapper.remap();
	}

	private DexBcClassRemapper(DexBcClass bcClass, BcClassRemapNameProvider renameProvider) {
		this.bcClass = bcClass;
		this.nameProvider = renameProvider;
	}

	private DexBcClass remap() {
		List<BcField> remappedFields = remapFields(bcClass.getFields());
		List<BcMethod> remappedMethods = remapMethods(bcClass.getMethods());

		bcClass.accept(new DexClassRemapper(remapped.getDexNode(), nameProvider));
		return remapped;
	}

	private List<BcField> remapFields() {

	}

	private List<BcMethod> remapMethods() {

	}

	protected final BcClassRemapNameProvider nameProvider;
	protected final DexBcClass bcClass;

	private class DexClassRemapper extends ClassRemapper {
		private DexClassRemapper(ClassVisitor cv, BcClassRemapNameProvider renameProvider) {
			super(cv, DexBcClassRemapNameProviderConverter.asAsmRemapper(renameProvider));
		...
		private class AsmMethodRemapper extends MethodRemapper {
			AsmMethodRemapper(MethodVisitor mv, BcClassRemapNameProvider renameProvider) {
				super(mv, DexBcClassRemapNameProviderConverter.asAsmRemapper(renameProvider));
			...
			protected final BcClassRemapNameProvider remapper;
