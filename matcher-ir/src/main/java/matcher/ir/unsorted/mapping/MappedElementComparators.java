...
public final class MappedElementComparators {
	public static <T extends MemberInstance<T>> Comparator<T> byName(NameType ns) {
		return new Comparator<T>() {
			...
			public int compare(T a, T b) {
				return compareNullLast(a.toSimpleName(ns), b.toSimpleName(ns));
			}
		};
	}

	public static <T extends Instance<T>> Comparator<T> byNameShortFirst(NameType ns) {
		return new Comparator<T>() {
			@Override
			public int compare(T a, T b) {
				String nameA = a.toSimpleName(ns);
				String nameB = b.toSimpleName(ns);
	...
	public static <T extends MemberInstance<?>> Comparator<T> byNameDescConcat(NameType ns) {
		return new Comparator<T>() {
			@Override
			public int compare(T a, T b) {
				String valA = Objects.toString(a.toSimpleName(ns)).concat(Objects.toString(a.getDesc(ns)));
				String valB = Objects.toString(b.toSimpleName(ns)).concat(Objects.toString(b.getDesc(ns)));
