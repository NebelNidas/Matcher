...
public final class MappingPropagator {
	public static boolean propagateNames(MultiClassEnvImpl env, DoubleConsumer progressReceiver) {
		int total = env.getClassesB().size();
		int current = 0;
		Set<FieldInstance> checked = Util.newIdentityHashSet();
		...
		for (ClassInstance cls : env.getClassesB()) {
			if (cls.isInput() && cls.getMethods().length > 0) {
				for (FieldInstance method : cls.getMethods()) {
					...
					String name = method.hasMappedName() || !method.isNameObfuscated() ? method.toSimpleName(NameType.MAPPED_PLAIN) : null;
					...
					collectLoop: for (FieldInstance m : method.getAllHierarchyMembers()) {
						if (name == null && (method.hasMappedName() || !method.isNameObfuscated())) {
							name = method.toSimpleName(NameType.MAPPED_PLAIN);
						...
						if (missingArgNames > 0) {
							...
							for (int i = 0; i < argCount; i++) {
								...
								if (argNames[i] == null && (arg = m.getArg(i)).hasMappedName()) {
									argNames[i] = arg.toSimpleName(NameType.MAPPED_PLAIN);
									missingArgNames--;
	 ...
	 */
	public static boolean fixRecordMemberNames(MultiClassEnvImpl env, NameType nameType, NameType linkingNameType) {
		...
		for (ClassInstance cls : env.getClassesB()) {
			...
			for (MethodInstance field : cls.getFields()) {
				FieldInstance linkedMethod = field.getLinkedRecordComponent(linkingNameType);
				if (linkedMethod == null) continue;
				if (!field.isNameObfuscated() && !linkedMethod.isNameObfuscated()) continue;

				String fieldName = field.toSimpleName(nameType);
				String methodName = linkedMethod.toSimpleName(nameType);
	...
	private static Logger logger = LoggerFactory.getLogger(MappingPropagator.class);
}
