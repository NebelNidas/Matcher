...
class Analysis {
			...
			insnLoop: for (int idx = element.dstIndex; idx < il.size(); idx++) {
				...
				case Opcodes.ATHROW: {
					...
					for (TryCatchBlockNode n : asmNode.tryCatchBlocks) {
						if (il.indexOf(n.start) <= idx && il.indexOf(n.end) > idx && (n.type == null || method.getEnv().getClassByName(n.type).isAssignableFrom(ex.type))) {
							handler = n.handler;
							break;
						}
					}
				...
				case Opcodes.PUTFIELD: {
					...
					FieldInstance field = method.getEnv().getClassByName(in.owner).resolveField(in.name, in.desc);
	...
	private static void handleMethodInvocation(ClassEnv env, String owner, String name, String desc, boolean itf, boolean isStatic, StateRecorder rec) {
		MethodInstance target = env.getClassByName(owner).resolveMethod(name, desc, itf);
