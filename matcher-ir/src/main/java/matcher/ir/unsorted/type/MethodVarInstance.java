...
public final class MethodVarInstance implements Instance<MethodVarInstance> {
	...
	public InstanceKind getKind() {
		return isArg ? InstanceKind.METHOD_ARG : InstanceKind.METHOD_VAR;
	}
	...
	public String getName(NameType type) {
		if (type == NameType.PLAIN) {
			...
			MultiClassEnvImpl env = method.cls.env.getGlobal();
			int uid = getUid();
